...
export default function Carousel({ items }) {
   ...
   const dotSize = 8;
   const dotPosition = totalPages > 1 ? (currentIndex / (totalPages - 1)) * (trackWidth - 18) + 3 : (dotSize / 2);
   
   // Calculate next position for stretch effect
   const nextIndex = isAnimating ? Math.max(0, Math.min(maxIndex, currentIndex + isAnimating)) : currentIndex;
   const nextDotPosition = totalPages > 1 ? (nextIndex / (totalPages - 1)) * (trackWidth - 18) + 3 : (dotSize / 2);
   ...
   return (
      <div className="relative">
         ...
         <div
            ...
            className={`flex scroll-smooth snap-x snap-mandatory -mx-4 min-h-[400px] overflow-x-hidden`}
         >
            {items.map((item) => (
               <div
                  key={item.id}
               ...
               >
                  <img src={item.image} alt={`Image ${item.id}`} className="w-full h-auto object-cover" />
               </div>
            ))}
         </div>

         {/* Sliding pagination indicator with stretch effect */}
         <div className="flex justify-center items-center mt-[18px] h-[16px]">
            <div className="relative rounded-full border-2 border-gray-300 h-[16px] w-[120px]">
               <div
                  ...
                  style={{ 
                     left: isAnimating === -1 ? `${nextDotPosition}px` : `${dotPosition}px`,
