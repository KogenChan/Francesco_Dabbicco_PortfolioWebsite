...
export default function RichText({ content }) {
  ...
  const renderText = (node) => {
    ...
    // Apply formatting - wrap progressively
    if (isCode) {
    ...
    switch (node.type) {
      ...
      case 'list':
        return node.listType === 'bullet' || node.tag === 'ul'
          ? <ul key={index} className="list-disc list-inside ml-4 mb-4 space-y-2">{children}</ul>
          : <ol key={index} className="list-decimal list-inside ml-4 mb-4 space-y-2">{children}</ol>;
      ...
      case 'quote':
        return (
          <blockquote key={index} className="border-l-4 border-gray-300 pl-4 py-2 mb-4 italic text-gray-700 dark:text-gray-300">
            {children}
          </blockquote>
        );
